...
import { UserPlus, LogIn, AtSign, X, User as UserIcon, Phone, Eye, EyeOff, CheckCircle2, ArrowRight, Zap } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || 'Gagal login. Periksa username dan password Anda.');
      }

      onLoginSuccess(data.user);
    } catch (err: any) {
      const msg = err?.message === 'Failed to fetch'
        ? 'Koneksi ke server terputus atau sedang bersiap. Silakan coba klik Log In lagi.'
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Do NOT automatically log in; switch to Login tab and prompt user to login
      setSuccessMessage(`Akun @${cleanUsername} berhasil dibuat! Silakan masukkan password untuk Log In.`);
      setMode('login');
      ...
      const msg = err?.message === 'Failed to fetch'
        ? 'Koneksi ke server terputus atau sedang bersiap. Silakan coba klik Daftar lagi.'
